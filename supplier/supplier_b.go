...
type MockySupplierB struct {
	MockySupplier
}

func (s *MockySupplierB) FetchData() ([]models.Hotel, error) {
	var hotels []models.Hotel
	var supplierB []SupplierB
	...
	// parse the data into supplier B model
	err = json.Unmarshal(data, &supplierB)
	if err != nil {
		return hotels, err
	}

	for _, hotel := range supplierB {
		rooms := []models.Image{}
		sites := []models.Image{}

		// tranforming supplier B images to our model
		...
		for _, img := range hotel.Images.Site {
			sites = append(sites, models.Image{
				Link:        img.Link,
				Description: img.Caption,
			})
		}
